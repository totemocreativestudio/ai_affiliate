...
export default function AdminDashboardV2({workspaceId}:{workspaceId:string}){
  ...
  const tabs=[["overview","Overview"],["users","Users"],["broadcast","Broadcast & Promo"],["content","Blog & Tutorial"],["social","Social"],["payments","Payments"],["ai","AI Usage"],["referral","Referral"],["issues","Bugs & UI"],["integrations","Integrations"]];

  return <section id="administration" className="legacy-page-anchor admin-owner-page">
    <div className="admin-owner-hero"><div><div className="eyebrow">LUMAWAY OWNER CONTROL</div><h1>Owner Dashboard</h1><p>Kelola bisnis Lumaway: user, broadcast, content, social, payment, AI usage, referral, error monitoring, dan integration.</p></div><button className="secondary" onClick={()=>load()}>Refresh Data</button></div>
