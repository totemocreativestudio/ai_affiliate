...
function parseCsv(text: string) {
  ...
  return rows.map((r) =>
    Object.fromEntries(headers.map((h, i) => [h || `Column ${i + 1}`, r[i] ?? ""]))
  );
}
...
async function loadXlsx() {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    s.async = true;
    s.onload = () => resolve();
    s.onerror = () => reject(new Error("Gagal memuat parser XLSX."));
...
export default function UploadCenter({ workspaceId }: Props) {
  ...
  async function parseFile(selected: File) {
    ...
    if (ext === "csv") {
      ...
      return { rows: parseCsv(text), buffer };
    }
  ...
  async function submit() {
    ...
    if (!startDate || !endDate) return setStatus("Start dan End Date wajib diisi.");
    setBusy(true);
    setResult(null);
    try {
      setStatus("Membaca file...");
      const parsed = await parseFile(file);
      if (!parsed.rows.length) throw new Error("File tidak memiliki data.");
      const hash = await fileHash(parsed.buffer);
      const importId = `IMP-${crypto.randomUUID().replace(/-/g, "").slice(0, 8).toUpperCase()}`;
      const batchSize = 300;
      ...
      for (let i = 0; i < totalBatches; i++) {
        ...
        last = await r.json();
        if (!r.ok || !last.ok) throw new Error(last.error || "Import gagal.");
      }
  ...
  return (
    <section id="upload" className="legacy-page-anchor">
      <div className="page-head">
        <div>
          <div className="eyebrow">DATA & UPLOAD</div>
          <h1>Upload Center</h1>
          <p className="muted">Mapping mengikuti Luma V5.6.4.1 dan selalu menulis ke workspace aktif.</p>
        </div>
      </div>
      <div className="card">
        <div className="grid">
        ...
        {status && <div className="flash success upload-status">{status}</div>}
