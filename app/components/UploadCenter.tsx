...
export default function UploadCenter({ workspaceId }: Props) {
 const supabase=createClient();const [dataType,setDataType]=useState("performance");const [platform,setPlatform]=useState("TikTok");const [startDate,setStartDate]=useState("");const [endDate,setEndDate]=useState("");const [storeName,setStoreName]=useState("");const [storeId,setStoreId]=useState("");const [file,setFile]=useState<File|null>(null);const [force,setForce]=useState(false);const [busy,setBusy]=useState(false);const [status,setStatus]=useState("");const [result,setResult]=useState<any>(null);const accept=useMemo(()=>".csv,.xlsx,.xls",[]);
 ...
 const uploadInfo:Record<string,{title:string;destination:string;mapping:string[]}>={
   performance:{title:"Affiliate Performance",destination:"Dashboard → Ranking Creator, Store Filter & Customer 360",mapping:platform==="Shopee"?["Store Name / Nama Toko (opsional; bisa diisi manual di form)","Affiliate ID","Affiliate Name","Affiliate Username","Sales(Rp)","Item Sold","Orders","Clicks","Est.Commission(Rp)","Total Buyers","New Buyers"]:["Store Name / Nama Toko (opsional; bisa diisi manual di form)","Creator name","GMV dari kreator","Pesanan teratribusi","Produk yang terjual dari kreator","Perkiraan komisi","Siaran LIVE","Video","Sampel","CTR","Impresi","Tayangan video"]},
   product_performance:{title:"Product Performance",destination:"Dashboard → Product Ranking, Store Filter & Master Data → Product Master",mapping:platform==="Shopee"?["Store Name / Nama Toko (opsional; bisa diisi manual di form)","Item id / Kode Item","Item Name / Nama Item","Price(Rp) / Harga(Rp)","Sales(Rp) / Omzet Penjualan(Rp)","Item Sold / Produk Terjual","Orders / Pesanan","Clicks","Est.Commission(Rp) / Estimasi Komisi(Rp)","ROI","Total Buyers / Total Pembeli","New Buyers / Pembeli Baru"]:["Store Name / Nama Toko (opsional; bisa diisi manual di form)","Product ID / Kode Produk","Product name / Nama Produk","GMV / Omzet Penjualan","Items sold","Est. commission","Samples","Sales creator","LIVE streams","Videos","Refunded GMV","Refunded items sold","Est. flat fee"]},
 ...
 return <section id="upload" className="legacy-page-anchor">
  ...
  <div className="card">
   ...
   {(!platformless||!periodless)&&<div className="upload-period-grid">
    ...
    {!periodless&&<><label>Start Date <span className="field-note">Opsional · dd/mm/yyyy</span><input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/></label><label>End Date <span className="field-note">Opsional · dd/mm/yyyy</span><input type="date" value={endDate} onChange={e=>setEndDate(e.target.value)}/></label>{(dataType==="performance"||dataType==="product_performance")&&<><label>Nama Toko <span className="field-note">Opsional · otomatis jika ada di file</span><input value={storeName} onChange={e=>setStoreName(e.target.value)} placeholder="Contoh: Gascomp Official"/></label><label>Store ID <span className="field-note">Opsional</span><input value={storeId} onChange={e=>setStoreId(e.target.value)} placeholder="ID toko marketplace"/></label></>}</>}
   </div>}
   ...
   <div className="owner-inline-note"><b>Multi-store:</b> satu file boleh berisi banyak toko jika tersedia kolom Store/Shop/Nama Toko. Jika file marketplace Anda hanya berisi satu toko tanpa kolom toko, isi Nama Toko di atas. Upload toko berikutnya tidak akan menghapus data toko sebelumnya pada periode yang sama.</div><label>File CSV/XLSX<input key={dataType} type="file" accept={accept} onChange={e=>setFile(e.target.files?.[0]||null)}/></label>
