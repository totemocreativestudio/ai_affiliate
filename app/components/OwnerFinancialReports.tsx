...
export default function OwnerFinancialReports({workspaceId}:{workspaceId:string}){
 ...
 return <div className="owner-section-stack">
  ...
  <section className="owner-panel"><h3>Report History</h3><div className="owner-table-wrap"><table><thead><tr><th>Type</th><th>Period</th><th>Model</th><th>Created</th><th></th></tr></thead><tbody>{reports.map(row=><tr key={row.id}><td>{labels[row.report_type]||row.report_type}</td><td>{row.period_start||"All"} → {row.period_end||"All"}</td><td>{row.model||"-"}</td><td>{new Date(row.created_at).toLocaleString("id-ID")}</td><td><button onClick={()=>void download(Number(row.id))}>Download PDF</button></td></tr>)}</tbody></table></div></section>
 </div>;
}
