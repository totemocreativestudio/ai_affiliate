...
export default function LegacyDashboard({ workspaceId }: Props) {
  ...
  async function load(targetPage=page){
    ...
    try{
      ...
      const {data:k,error:ke}=await supabase.rpc("get_dashboard_kpi",{p_workspace_id:workspaceId,p_start_date:start||null,p_end_date:end||null,p_platform:platform||null,p_creator_id:null});
      if(ke)throw ke;
      setKpi(k?.[0]||{total_creators:0,total_sales_records:0,total_qty:0,total_orders:0,total_gmv:0,total_commission:0});
      const {data:r,error:re}=await supabase.rpc("get_creator_ranking",{p_workspace_id:workspaceId,p_start_date:start||null,p_end_date:end||null,p_platform:platform||null,p_creator_id:null,p_search:null,p_page:targetPage,p_page_size:50});
      if(re)throw re;
  ...
  return <section id="dashboard" className="legacy-page-anchor dashboard-page">
    ...
    <div className="grid"><div className="card"><h3>Platform</h3><p className="muted">Filter platform aktif: <b>{platform||"All"}</b>.</p></div><div className="card"><h3>Target & KPI Bulanan</h3><p className="muted">Tanggal kosong berarti seluruh data workspace.</p></div></div>
  </section>;
}
