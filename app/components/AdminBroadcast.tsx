...
export default function AdminBroadcast({workspaceId}:{workspaceId:string}){
  ...
  const [form,setForm]=useState({category:"education",concept:"",goal:"",title:"",body:"",action_label:"Buka",action_url:"#content-hub",image_prompt:"",image_url:"",scope:"global",channel:"web"});
  ...
  async function publish(){if(!form.title||!form.body)return setStatus("Title dan body wajib diisi.");setStatus("Mempublikasikan ke user...");const d=await call("publish",form);if(d){const wa=d.whatsapp?.attempted?` · WhatsApp ${d.whatsapp.sent}/${d.whatsapp.attempted} terkirim`:"";setStatus(`Broadcast ${d.id?"#"+d.id:""} dipublikasikan${wa}.`);await load()}}
