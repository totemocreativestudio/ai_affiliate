...
export default function UserProfile({workspaceId,userId}:{workspaceId:string;userId:string}){
  ...
  const [form,setForm]=useState<Row>({full_name:"",position_title:"",bio:"",education:"",birth_date:""});
  ...
  async function load(){
    const [p,s]=await Promise.all([
      supabase.from("profiles").select("id,email,full_name,phone,phone_verified_at,avatar_url,bio,education,birth_date,position_title,social_alias,social_avatar_key,whatsapp_opt_in,whatsapp_opt_in_at,whatsapp_channel_joined_at,updated_at").eq("id",userId).single(),
