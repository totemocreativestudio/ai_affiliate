...
export default function UserProfile({workspaceId,userId}:{workspaceId:string;userId:string}){
  ...
  async function saveProfile(){
    ...
    const completed=Boolean(emailVerifiedAt&&phoneVerifiedAt&&profile?.password_configured_at);
  ...
  async function verifyPhone(){
    ...
    try{
      ...
      const {data:{user}}=await supabase.auth.getUser();
      const emailVerifiedAt=(user as any)?.email_confirmed_at||profile?.email_verified_at||null;
      const completed=Boolean(emailVerifiedAt&&profile?.password_configured_at&&String(form.full_name||"").trim()&&String(form.nickname||"").trim()&&String(form.education||"").trim()&&String(form.birth_date||"").trim()&&String(form.bio||"").trim());
  ...
  async function setAccountPassword(){
    ...
    if(newPassword.length<8)return setStatus("Password minimal 8 karakter.");
    if(newPassword!==confirmPassword)return setStatus("Konfirmasi password belum sama.");
    setPasswordBusy(true);setStatus("");
    const {error}=await supabase.auth.updateUser({password:newPassword});
    if(error){setPasswordBusy(false);return setStatus(error.message)}
    const canComplete=Boolean(profile?.email_verified_at&&profile?.phone_verified_at&&String(form.full_name||"").trim()&&String(form.nickname||"").trim()&&String(form.education||"").trim()&&String(form.birth_date||"").trim()&&String(form.bio||"").trim());
    if(canComplete)await supabase.from("profiles").update({profile_completed:true,updated_at:new Date().toISOString()}).eq("id",userId);
    setNewPassword("");setConfirmPassword("");setPasswordBusy(false);
    setStatus("Password login berhasil dibuat. Sekarang akun dapat masuk dengan Google maupun email + password.");
    window.dispatchEvent(new Event("lumaway-profile-updated"));
    await load();
  }

  async function toggleWeeklyEmail(enabled:boolean){
    if(weeklyBusy)return;
    setWeeklyBusy(true);
    const {error}=await supabase.from("profiles").update({
      weekly_email_enabled:enabled,
      weekly_email_opt_out_at:enabled?null:new Date().toISOString(),
      updated_at:new Date().toISOString()
    }).eq("id",userId);
    setWeeklyBusy(false);
    if(error)return setStatus(error.message);
    setStatus(enabled?"Email insight mingguan diaktifkan.":"Email insight mingguan dinonaktifkan.");
    await load();
  }
  ...
  const accountComplete=Boolean(profile?.profile_completed&&profile?.password_configured_at);
  return <section id="profile" className="legacy-page-anchor profile-page">
    <div className="eyebrow">ACCOUNT</div><h1>My Profile</h1>{!accountComplete&&<div className="profile-completion-notice"><strong>Lengkapi keamanan akun sebelum melanjutkan</strong><span>Isi data wajib, verifikasi email & WhatsApp, lalu buat password. Akun Google tetap dapat login lewat Google dan juga email + password.</span></div>}<p className="muted">Data pribadi Anda terpisah dari profil Community. Nama dan foto Community dapat Anda atur sendiri tanpa menampilkan email atau nomor WhatsApp.</p>
    {accountComplete&&<div className="profile-complete-success"><strong>Selamat profil kamu sudah lengkap.</strong><span>Kamu dapat menggunakan seluruh fitur Lumaway sesuai masa aktif akun.</span></div>}
    <div className="profile-layout">
      ...
      <div>
        ...
        <div className="card"><div className="profile-card-head"><div><h3>Email Insight Mingguan</h3><p className="muted">Setiap minggu Lumaway menyiapkan ringkasan penjualan, top creator, dan promo aktif dari workspace Anda.</p></div><label className="inline-check"><input type="checkbox" checked={profile?.weekly_email_enabled!==false} disabled={weeklyBusy} onChange={e=>void toggleWeeklyEmail(e.target.checked)}/>{profile?.weekly_email_enabled!==false?"Aktif":"Nonaktif"}</label></div></div>
    ...
    {status&&<div className={`flash ${/^(Selamat|Data profil tersimpan|Foto profil diperbarui|Profil Community berhasil diperbarui|Foto profil Community berhasil diperbarui|Link verifikasi email sudah dikirim|Password login berhasil dibuat|Email insight mingguan|OTP 6 digit dikirim|Nomor WhatsApp terverifikasi|Nomor WhatsApp berhasil diverifikasi|Status bergabung)/i.test(status)?"success":"error"}`}>{status}</div>}
  </section>;
}
