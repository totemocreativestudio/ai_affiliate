...
export async function GET(req:NextRequest){
  ...
  let post:SharePost|null=null;
  if(Number.isFinite(id)&&id>0){
    try{const supabase=createPublicClient();const {data}=await supabase.rpc("luma_get_public_share_post",{p_post_id:id});if(Array.isArray(data)&&data.length)post=data[0] as SharePost}catch{}
  }
  ...
  const date=post?.created_at?new Date(post.created_at).toLocaleString("id-ID",{timeZone:"Asia/Jakarta"}):"";
  const html=`<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Lumaway Community</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${esc(canonical)}"><meta property="og:type" content="article"><meta property="og:site_name" content="Lumaway"><meta property="og:title" content="Lumaway Community"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${esc(canonical)}"><meta property="og:image" content="${esc(image)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="Lumaway Community"><meta name="twitter:description" content="${esc(description)}"><meta name="twitter:image" content="${esc(image)}"><style>*{box-sizing:border-box}body{margin:0;min-height:100vh;background:#f6f7fb;color:#172033;font-family:Arial,Helvetica,sans-serif;display:grid;place-items:center;padding:24px}.card{width:min(680px,100%);background:#fff;border:1px solid #e4e7ec;border-radius:22px;overflow:hidden;box-shadow:0 24px 70px rgba(15,23,42,.12)}.hero{display:block;width:100%;max-height:460px;object-fit:cover}.body{padding:26px}.brand{display:flex;align-items:center;gap:10px;margin-bottom:18px}.brand img{width:34px;height:34px}.brand b{display:block;letter-spacing:.08em}.brand small,.date{color:#98a2b3}.copy{font-size:18px;line-height:1.65}.date{display:block;margin:0 0 18px}.cta{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:0 16px;border-radius:11px;background:#635bff;color:#fff;text-decoration:none;font-weight:700}</style></head><body><article class="card">${post?.image_url?`<img class="hero" src="${esc(post.image_url)}" alt="Lumaway Community post">`:""}<div class="body"><div class="brand"><img src="/luma-mark.png" alt="Lumaway"><div><b>LUMAWAY</b><small>Community</small></div></div><p class="copy">${esc(post?.body||"Post Lumaway Community tersedia untuk dilihat di aplikasi.")}</p><small class="date">${esc(date)}</small><a class="cta" href="${esc(openUrl)}">Buka di Lumaway</a></div></article></body></html>`;
