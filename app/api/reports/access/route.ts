...
export async function POST(req:NextRequest){
  try{
    ...
    if(action==="preview"){
      ...
      const {data:fresh}=await ctx.admin.from("luma_pdf_reports").select("id,title,period_start,period_end,document_json,previewed_at,downloaded_at,download_count,watermark_removed_at,page_count,created_at").eq("id",reportId).single();
