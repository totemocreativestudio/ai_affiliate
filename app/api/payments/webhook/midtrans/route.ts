...
export async function POST(req:NextRequest){
 try{
  ...
  if(success){
    ...
    if(subOrder){const {data:result,error}=await admin.rpc("luma_complete_subscription",{p_order_code:orderCode,p_payment_reference:paymentRef||null,p_provider_payload:body});if(error)throw error;if(subOrder.checkout_intent_id)await admin.from("luma_payment_checkout_intents").update({status:"paid",payment_reference:paymentRef||null,updated_at:new Date().toISOString()}).eq("id",subOrder.checkout_intent_id);if(!result?.already_paid)await sendExternalCustomerNotice(admin,{userId:subOrder.user_id,workspaceId:subOrder.workspace_id,kind:"subscription_paid",title:"Langganan Lumaway aktif",message:`Pembayaran ${orderCode} via Midtrans berhasil. Paket ${subOrder.luma_subscription_plans?.name||"Lumaway"} senilai ${money(subOrder.amount)} telah aktif.`,actionUrl:app});return NextResponse.json({ok:true,type:"subscription",result})}
    const {data:result,error}=await admin.rpc("luma_complete_topup",{p_order_code:orderCode,p_payment_reference:paymentRef||null,p_provider_payload:body});if(error)throw error;if(tokenOrder.checkout_intent_id)await admin.from("luma_payment_checkout_intents").update({status:"paid",payment_reference:paymentRef||null,updated_at:new Date().toISOString()}).eq("id",tokenOrder.checkout_intent_id);if(!result?.already_paid)await sendExternalCustomerNotice(admin,{userId:tokenOrder.user_id,workspaceId:tokenOrder.workspace_id,kind:"token_paid",title:"Top up token berhasil",message:`Pembayaran ${orderCode} via Midtrans berhasil. ${Number(tokenOrder.package_tokens||0)} token telah ditambahkan.`,actionUrl:app});return NextResponse.json({ok:true,type:"token",result});
  }
  if(["expire","cancel","deny","failure"].includes(tx)){const table=subOrder?"luma_subscription_orders":"luma_topup_orders";const mapped=tx==="expire"?"expired":tx==="cancel"?"canceled":"failed";await admin.from(table).update({status:mapped,provider_payload:body}).eq("order_code",orderCode).neq("status","paid");if(order.checkout_intent_id)await admin.from("luma_payment_checkout_intents").update({status:mapped==="expired"?"expired":"failed",updated_at:new Date().toISOString()}).eq("id",order.checkout_intent_id)}
