-- Dashboard KPI must aggregate the same workspace rows as ranking/store RPCs.
-- RLS on public.sales can intentionally hide creator rows from direct selects,
-- so this RPC uses SECURITY DEFINER with an explicit workspace access check.
...
begin
  ...
  agg as (
    select
      ...
      count(distinct nullif(coalesce(sku,product_name),''))::bigint products,
      coalesce(sum(cost_product),0) cost_product,
      coalesce(sum(shipping_cost),0) shipping,
      coalesce(sum(ads_spend),0) ads,
  ...
  select
    ...
    case when (a.cost_product+a.shipping+a.ads+a.commission)>0
      then round(a.gmv/(a.cost_product+a.shipping+a.ads+a.commission),2)
      else 0 end,
